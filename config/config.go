...
func GenerateMachineId() string {
	...
	if _, err := rand.Read(bytes); err != nil {
		// crypto/rand failures are unrecoverable on a sane system; falling back
		// to a deterministic value would weaken request fingerprinting, so we
		// surface the error by panicking. Callers run at startup or on admin
		// requests, both of which can safely abort.
...
// generateRandomPassword returns a 24-character URL-safe random string.
// Used to seed a fresh installation instead of the well-known "changeme" default.
func generateRandomPassword() string {
	bytes := make([]byte, 18) // 18 bytes -> 24 base64url chars
	if _, err := rand.Read(bytes); err != nil {
		panic(fmt.Sprintf("crypto/rand unavailable: %v", err))
	}
...
// FirstRunPassword 是首次创建配置文件时生成的临时密码，
// 用于 main.go 在启动日志中打印一次提示，方便管理员在首次部署时获取。
// 后续启动加载已有配置文件时该值为空。
...
func Load() error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			// Create default configuration with a random admin password.
