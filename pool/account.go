...
func (p *AccountPool) GetNext() *config.Account {
	...
	for i := 0; i < n; i++ {
		...
		// Token 已过期（不仅是即将过期）才跳过；handler 的 ensureValidToken
		// 会在请求路径上自动刷新即将过期的 token，没必要在路由阶段把它们
		// 当成不可用账号屏蔽掉，否则所有 token 同时进入刷新窗口时会导致
		// "No available accounts"。
		if acc.ExpiresAt > 0 && acc.RefreshToken == "" && time.Now().Unix() > acc.ExpiresAt {
			seen[acc.ID] = true
			continue
		}
...
func (p *AccountPool) GetNextForModel(model string) *config.Account {
	...
	for i := 0; i < n; i++ {
		...
		// 已过期且无法刷新（无 RefreshToken）才跳过；其余情况让 handler
		// 路径上的 ensureValidToken 自动刷新，避免 token 即将过期就把所有
		// 账号判定为不可用。
		if acc.ExpiresAt > 0 && acc.RefreshToken == "" && time.Now().Unix() > acc.ExpiresAt {
			seen[acc.ID] = true
			continue
