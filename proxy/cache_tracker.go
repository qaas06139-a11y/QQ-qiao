...
// ForgetAccount drops every cached fingerprint for the given account so the
// memory used to track its prompt-cache fingerprints is released. Safe to call
// when an account is deleted from the pool or disabled long-term.
func (t *promptCacheTracker) ForgetAccount(accountID string) {
	if t == nil || accountID == "" {
		return
	}
	t.mu.Lock()
