...
// defaultFallbackModel is the Kiro model used when a request specifies a
// model name we can't recognise (e.g. Codex sending gpt-5-codex). Without
// this, the account-pool lookup filters every account out and the proxy
// returns "No available accounts" even though the pool is healthy.
const defaultFallbackModel = "claude-sonnet-4.5"
...
func ParseModelAndThinking(model string, thinkingSuffix string) (string, bool) {
	...
	// 已经是有效的 Kiro 模型 (claude-*) 直接放行；其余模型（"auto"、deepseek
	// 等 Kiro 后端模型 + Codex 发的未知 gpt-* / o-* 都会落到 fallback。
	if strings.HasPrefix(lower, "claude-") {
		return model, thinking
	}

	// 未知模型：直接当成默认 Sonnet。Codex 不知道 Kiro 内部模型清单，会发
	// gpt-5-codex / gpt-4.1 / o3-mini 之类的名字；如果照搬这些名字给账号池
	// 路由，accountHasModel 会全部过滤掉，导致 503。
	if model == "" {
		return defaultFallbackModel, thinking
	}
	if !isLikelyKiroModel(lower) {
		return defaultFallbackModel, thinking
	}
	return model, thinking
}

// isLikelyKiroModel returns true when the model id looks like one that the
// Kiro backend itself exposes (claude-*, deepseek-*, glm-*, qwen-*, minimax-*,
// auto). Used as a heuristic to decide whether to forward the name as-is or
// fall back to the default Sonnet.
func isLikelyKiroModel(lower string) bool {
	prefixes := []string{
		"claude-",
		"deepseek-",
		"glm-",
